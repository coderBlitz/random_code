...
// Compatible with "message_server" AND "continuous_server"

int main(){
	char msg[101];// Message being sent
	char *ip = "127.0.0.1";
	int port = 2000;
	...
	int sent = write(client,msg,strlen(msg));// Send message
	if(sent != strlen(msg)) fprintf(stderr,"Something happened while sending the message!\n");
