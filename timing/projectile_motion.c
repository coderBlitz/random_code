...
int main(int argc,char **argv){
	...
	double START_Y_POS;// Height at which object is launched from
	double INIT_VELOCITY;// Velocity in generic measurement (distance unit/second)
	double INIT_ANGLE;// Launch angle in degrees
	...
	double x_pos=0, y_pos = START_Y_POS;// Will be current stats of projectile in metres
	double X_VELOCITY = INIT_VELOCITY * cos(INIT_ANGLE * M_PI/180.0);// Get horizontal speed
	double Y_VELOCITY = INIT_VELOCITY * sin(INIT_ANGLE * M_PI/180.0);// Get vertical speed
	double gravity = -9.8/pow((y_pos+6400)/6400,2);// Get gravity at different altitudes above surface
	long surface = 0;// Will be how far down the surface of the earth is, changes based on distance

	printf("INITIAL HEIGHT: %.03lfm\n",START_Y_POS);
	printf("INITIAL SPEED: %.03lfm/s\n",INIT_VELOCITY);
	printf("ANGLE: %.03lf degrees\n",INIT_ANGLE);
	printf("LAUNCH!\n");
	...
	while(y_pos >= surface){// While projectile is still in flight
		...
		printf("\rTime: %5.04lfs\t  X_POS: %11.5lfm\t Y_POS: %8.06lfm",cur_time,x_pos,y_pos);
		printf("\tSurface: %ld",surface);
		fflush(stdout);
if(y_pos<0) printf("\n\nGravity: %5.06lf\t  X_POS: %11.5lfm\t Y_POS: %8.06lfm\n\n",gravity,x_pos,y_pos);
	}
	gettimeofday(&tval_current,NULL);
	timersub(&tval_current,&tval_start,&tval_diff);
	cur_time = tval_diff.tv_sec + (tval_diff.tv_usec / 1e6);

	printf("\nLANDED\nProjectile went %.04lf distance units in %.06lf seconds.\n",x_pos,cur_time);
}
