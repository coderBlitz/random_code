...
int main(){
/* Note:	Most of the calculations here are meant to be re-calculated inside
			 of an event loop. Here they are just present to demonstrate algorithm.
			The turn directions here are compass-based, so positive turn rate
...
*/
	...
	struct position_t ent = {10.0, 10.0, 0.0};
	...
	printf("Radius min: %.4lf\n", radius_min);

	unsigned long count = 0;
	unsigned long N = 240/delay_fraction;
	while(++count <= N){
		...
		if(diff > radius_min){
			...
			if(lr_cross > 0){
				if(dd > -velocity/mag/2)
					...
					db = 0; // How much to turn out when heading toward radius
				}
			}else if(lr_cross < 0){
				if(dd > -velocity/mag/2)
					db = mag;
			...
			if(lr_cross > 0){
				if(dd < 0)
					...
					db *= 0.3; // Rate at which to turn inward, when close to radius
			}else if(lr_cross < 0){
				if(dd < 0)
					db = -mag;
				else if(db > 0)
