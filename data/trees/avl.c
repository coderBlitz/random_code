...
static inline void updateHeight(struct node *root){
	...
	uint8_t left = (root->left != NULL) ? left = root->left->height : 0;
	uint8_t right = (root->right != NULL) ? right = root->right->height : 0;
...
int rotate(struct node **tree){
	...
	uint8_t left = ((*tree)->left == NULL) ? 0 : (*tree)->left->height;
	uint8_t right = ((*tree)->right == NULL) ? 0 : (*tree)->right->height;
